...
public partial class ACADEMIC_SemesterOffer : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            else
            {
                ...
                objCommon.FillDropDownList(ddlSessionCopy, "ACD_SESSION_MASTER", "SESSIONNO", "SESSION_NAME", "", "SESSIONNO DESC");
                //objCommon.FillDropDownList(ddlSession, "ACD_SESSION_MASTER", "SESSIONNO", "SESSION_NAME", "", "SESSIONNO DESC");
                //objCommon.FillDropDownList(ddlSessionCopy, "ACD_SESSION_MASTER", "SESSIONNO", "SESSION_NAME", "", "SESSIONNO DESC");
                //objCommon.FillDropDownList(ddlSemester, "ACD_SEMESTER", "SEMESTERNO", "SEMESTERNAME", "SEMESTERNO>0", "SEMESTERNAME ");
                objCommon.FillDropDownList(ddlSemesterCopy, "ACD_SEMESTER", "SEMESTERNO", "SEMESTERNAME", "SEMESTERNO>1", "SEMESTERNAME ");
    ...
    protected void btnShowCopy_Click(object sender, EventArgs e)
    {
        ...
        ShowDetailsCopy(Convert.ToInt32(ddlCollegeSchNameC.SelectedValue), Convert.ToInt32(ddlStudyLevelC.SelectedValue), Convert.ToInt32(ddlSessionCopy.SelectedValue), Convert.ToInt32(ddlClassificationC.SelectedValue));
    ...
    private void ShowDetails()
    {
        try
        {
            ...
            DataSet ds = objmp.GetCampusForAllotSemester(Convert.ToInt32(ddlColg.SelectedValue), Convert.ToInt32(ddlProgClassification.SelectedValue), Convert.ToInt32(ddlAwardingInstitute.SelectedValue), Convert.ToInt32(ddlSemester.SelectedValue), Convert.ToInt32(ddlSession.SelectedValue));
    ...
    private void ShowDetailsCopy(int clg, int ugpg, int Sessions, int classification_no)
    {
        try
        {
            btnCancelCopy.Visible = true;
            lnkSubmit.Visible = true;
            if(ddlSessionCopy.SelectedValue=="0")
            {
                lvOfferCopy.DataSource = null;
                lvOfferCopy.DataBind();
                objCommon.DisplayMessage(this.updGradeEntry, "Please Select Session!", this.Page);
                return;
            }
            if (ddlSemesterCopy.SelectedValue == "0")
            {
                lvOfferCopy.DataSource = null;
                lvOfferCopy.DataBind();
                objCommon.DisplayMessage(this.updGradeEntry, "Please Select Semester!", this.Page);
                return;
            }
            ...
            string HtmlString = string.Empty;
            int i = 0;
            DataSet ds = objmp.GetSemesterCampusForAllot(clg, ugpg, classification_no, Sessions, Convert.ToInt32(ddlToSessionCopy.SelectedValue), Convert.ToInt32(ddlSemesterCopy.SelectedValue));
            lvOfferCopy.DataSource = ds;
            lvOfferCopy.DataBind();
            foreach (ListViewDataItem item in lvOfferCopy.Items)
            {
                CheckBox chk = item.FindControl("chkSelect") as CheckBox;

                HiddenField CopyStatus = item.FindControl("CopyStatus") as HiddenField;
                    int status=Convert.ToInt32(CopyStatus.Value);
                    if (status == 1)
                    {
                        chk.Checked = true;
                    }
    ...
    {
        if (ddlColg.SelectedValue != "0")
        {
            objCommon.FillDropDownList(ddlProgClassification, "ACD_COLLEGE_DEGREE_BRANCH CD INNER JOIN   ACD_UA_SECTION  S ON(CD.UGPGOT=S.UA_SECTION)", "DISTINCT UA_SECTION", "ISNULL(UA_SECTIONNAME,'') UA_SECTIONNAME", "UA_SECTION > 0 AND COLLEGE_ID=" + Convert.ToInt32(ddlColg.SelectedValue), "UA_SECTION");
        }
        else
        {
            ddlProgClassification.Items.Clear();
        }
        ddlAdmBatch.SelectedIndex = 0;
      //  ddlProgClassification.SelectedIndex = 0;
        ddlAwardingInstitute.SelectedIndex = 0;
    ...
    {
        if (ddlProgClassification.SelectedValue != "0" && ddlColg.SelectedValue != "0")
        {
            objCommon.FillDropDownList(ddlAwardingInstitute, "ACD_COLLEGE_DEGREE_BRANCH CD INNER JOIN   ACD_AFFILIATED_UNIVERSITY  S ON(S.AFFILIATED_NO =CD.AFFILIATED_NO)", " DISTINCT S.AFFILIATED_NO", "AFFILIATED_SHORTNAME", "AREA_INT_NO > 0 AND COLLEGE_ID=" + Convert.ToInt32(ddlColg.SelectedValue) + "AND UGPGOT=" + Convert.ToInt32(ddlProgClassification.SelectedValue) + "", "AFFILIATED_SHORTNAME");
        }
        else
        {
    ...
    {
        if (ddlAwardingInstitute.SelectedValue != "0")
        {
            objCommon.FillDropDownList(ddlSemester, "ACD_SEMESTER", "SEMESTERNO", "SEMESTERNAME", "SEMESTERNO>1", "SEMESTERNAME ");
        }
        else
        {
      ...
      protected void ddlSemester_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlSemester.SelectedValue != "0" && ddlSession.SelectedValue != "0" && ddlColg.SelectedValue != "0" 
                && ddlProgClassification.SelectedValue!="0" && ddlAwardingInstitute.SelectedValue!="0")
            {
                divUGPG.Visible = true;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string Degreeno = string.Empty; string Branchno = string.Empty; string SemesterFees = "";
                ...
                TextBox SemesterFee = item.FindControl("txtSemesterFees") as TextBox;
    ...
    protected void btnSaveCopy_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string Branch = "";
            int ck1 = 0;
            foreach (ListViewDataItem dataitem in lvOfferCopy.Items)
            {
                CheckBox chkSelect = dataitem.FindControl("chkSelect") as CheckBox;
                HiddenField hdnDegreeno = dataitem.FindControl("hdnDegreeno") as HiddenField;
                HiddenField hdnBranchNo = dataitem.FindControl("hdnBranchNo") as HiddenField;
          
                if (chkSelect.Checked == true)
                {
                    int Degreeno = Convert.ToInt32(hdnDegreeno.Value);
                     int BranchNo= Convert.ToInt32(hdnBranchNo.Value);
                     Branch += BranchNo + ',';
                    ck1 = objmp.addCampusSessionCopy(Convert.ToInt32(ddlSessionCopy.SelectedValue), Convert.ToInt32(ddlToSessionCopy.SelectedValue),Degreeno,BranchNo,Convert.ToInt32(ddlSemesterCopy.SelectedValue), Convert.ToInt32(Session["userno"].ToString()));
                }
                
            }
            if (Branch== "")
            {
                objCommon.DisplayMessage(this.updGradeEntry, "Plese Check At List One", this.Page);
            }
            if (ck1 == 1)
            {
    ...
    protected void lvCampusWiseIntake_ItemDataBound1(object sender, ListViewItemEventArgs e)
    {
        if ((e.Item.ItemType == ListViewItemType.DataItem))
        {
            ...
            if (camp1 == "0")
            {
                Script += "$('#myTable th:nth-child(3)').hide();$('#myTable td:nth-child(2)').hide();";
                Script += "arrOfHiddenColumns.push(2);";
            }
            if (camp2 == "0")
            {
                Script += "$('#myTable th:nth-child(4)').hide();$('#myTable td:nth-child(3)').hide();";
                Script += "arrOfHiddenColumns.push(3);";
            }
            if (camp3 == "0")
            {
                Script += "$('#myTable th:nth-child(5)').hide();$('#myTable td:nth-child(4)').hide();";
            ...
            if (camp9 == "0")
            {
                Script += "$('#myTable th:nth-child(11)').hide();$('#myTable td:nth-child(10)').hide();";
                Script += "arrOfHiddenColumns.push(10);";
            }
            if (camp10 == "0")
            {
                Script += "$('#myTable th:nth-child(12)').hide();$('#myTable td:nth-child(11)').hide();";
    ...
    protected void lvOfferCopy_ItemDataBound1(object sender, ListViewItemEventArgs e)
    {
        if ((e.Item.ItemType == ListViewItemType.DataItem))
        {
            ...
            if (camp6 == "0")
            {
                Script += "$('#myTable2 th:nth-child(9)').hide();$('#myTable2 td:nth-child(8)').hide();";
                Script += "arrOfHiddenColumns.push(8);";
            }
            if (camp7 == "0")
            {
                Script += "$('#myTable2 th:nth-child(10)').hide();$('#myTable2 td:nth-child(9)').hide();";
                Script += "arrOfHiddenColumns.push(9);";
            }
            if (camp8 == "0")
            {
                Script += "$('#myTable2 th:nth-child(11)').hide();$('#myTable2 td:nth-child(10)').hide();";
                Script += "arrOfHiddenColumns.push(10);";
            }
            if (camp9 == "0")
            {
                Script += "$('#myTable2 th:nth-child(12)').hide();$('#myTable2 td:nth-child(11)').hide();";
                Script += "arrOfHiddenColumns.push(11);";
            }
            if (camp10 == "0")
            {
                Script += "$('#myTable2 th:nth-child(13)').hide();$('#myTable2 td:nth-child(12)').hide();";
                Script += "arrOfHiddenColumns.push(12);";
            }
    ...
    public void ClearOfferCopy()
    {
        ...
        ddlCollegeSchNameC.SelectedIndex = 0;
        ddlSessionCopy.SelectedIndex = 0;
        ddlStudyLevelC.SelectedIndex = 0;
        ddlSemesterCopy.SelectedIndex = 0;
        ddlToSessionCopy.Items.Clear();
    ...
    protected void ddlSessionCopy_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlSessionCopy.SelectedValue != "0")
        {
            objCommon.FillDropDownList(ddlToSessionCopy, "ACD_SESSION_MASTER", "SESSIONNO", "SESSION_NAME", "SESSIONNO >" + Convert.ToInt32(ddlSessionCopy.SelectedValue), "SESSIONNO DESC");
        }
        else
        {
            ddlToSessionCopy.Items.Clear();
    ...
    protected void ddlToSessionCopy_SelectedIndexChanged(object sender, EventArgs e)
    {
        lvOfferCopy.DataSource = null;
