...
public partial class ACADEMIC_OnlinepaymentStudentLogin : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    ...
                    ViewState["userno"] = Convert.ToInt32(objCommon.LookUp("ACD_STUDENT", "DISTINCT USERNO", "IDNO=" + Convert.ToInt32(Session["idno"])));//objCommon.LookUp("ACD_USER_REGISTRATION", "USERNO", "USERNAME='" + Session["username"].ToString() + "'");
                    PopulateDropDown();
            ...
            ViewState["RESULT_STATUS"] = objCommon.LookUp("ACD_STUDENT_RESULT", "COUNT(1)", "IDNO=" + Convert.ToInt32(Session["idno"]) + " AND SEMESTERNO=" + ViewState["semesterno"] + " AND SESSIONNO=" + ViewState["SESSIONNO"] + "");
    ...
    private bool CheckActivity()
    {
        ...
        DataSet ds = feeController.GetSemesterRegistrationActivityStatus(Convert.ToInt32(Session["idno"]));
        ...
        try
        {
            if (Session["usertype"].ToString().Equals("2"))     //Student 
            {
                ...
                if (dsStudent != null && dsStudent.Tables.Count > 0)
                {
                    ...
                    string payment = objCommon.LookUp("ACD_DCR", "(isnull(SESSIONNO,0))", "IDNO=" + Convert.ToInt32(Session["idno"]) + " AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"]) + " AND RECIEPT_CODE=" + "'" + Convert.ToString("DP") + "'");
                    if (payment != "")
                    {
    ...
    private void GetPreviousReceipt()
    {
        ...
        string SP_Name2 = "PKG_GETS_PAID_FEES_DETAILS_FOR_PAYMENT_FOR_DOWNPAYMENT";
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)   ////Student Payment Click Button
    {
        try
        {
            ...
            else
            {
                ...
                string session = string.Empty;
                session = GetSession();
                int result = 0; int DcrTempNo = 0 ,checkpay=0;
                int DM_NO = 0;
                ...
                //checkpay = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TEMP", "COUNT(1)", "IDNO=" + Convert.ToInt32(Session["idno"]) + "(DOC_FILENAME IS NOT NULL OR DOC_FILENAME = '') AND PAY_SERVICE_TYPE = 5 AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"]) + "and RECIEPT_CODE=" + "'" + Convert.ToString(ViewState["RECIEPT_CODE"]) + "'"));
                checkpay = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TEMP", "COUNT(1)", "IDNO=" + Convert.ToInt32(Session["idno"]) + "AND (DOC_FILENAME IS NOT NULL OR DOC_FILENAME != '') AND PAY_SERVICE_TYPE = 5 AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"]) + "and RECIEPT_CODE=" + "'" + Convert.ToString(ViewState["RECIEPT_CODE"]) + "'"));
                if (checkpay > 0)
                {
                    objCommon.DisplayMessage(updBulkReg, "Deposit Slip Already Uploaded Unable to To Pay In Cash !!!", this.Page);
                    return;
                }
    ...
    protected void lkpayment_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Convert.ToString(ds.Tables[3].Rows[0]["MANDATORY_DOC_COUNT"]) == "1")
            {
    ...
    private void PopulateDropDown()
    {
        try
        {
            string SP_Name = "PKG_ACD_FILL_DROP_DOWN_DETAILS";
            string SP_Parameters = "@P_DYNAMIC_FILTER";
            ...
            ddlProgramTypes.Items.Clear();
            ddlProgramTypes.Items.Add("Please Select");
            ddlProgramTypes.SelectedItem.Value = "0";

            ddlOnlineMobileCode.Items.Clear();
            ddlOnlineMobileCode.Items.Add("Please Select");
            ddlOnlineMobileCode.SelectedItem.Value = "0";

            ddlPCon.Items.Clear();
            ddlPCon.Items.Add("Please Select");
            ddlPCon.SelectedItem.Value = "0";

            ddlHomeMobileCode.Items.Clear();
            ddlHomeMobileCode.Items.Add("Please Select");
            ddlHomeMobileCode.SelectedItem.Value = "0";

            ddlReceiptType.Items.Clear();
            ddlReceiptType.Items.Add("Please Select");
            ddlReceiptType.SelectedItem.Value = "0";
            ...
            {
                ddlProgramTypes.DataSource = ds.Tables[1];
                ddlProgramTypes.DataValueField = "UA_SECTION";
                ddlProgramTypes.DataTextField = "UA_SECTIONNAME";
                ddlProgramTypes.DataBind();
            }
            if (ds.Tables[2] != null && ds.Tables[2].Rows.Count > 0)
            {
                ddlOnlineMobileCode.DataSource = ds.Tables[2];
                ddlOnlineMobileCode.DataValueField = "COUNTRYNO";
                ddlOnlineMobileCode.DataTextField = "COUNTRYNAME";
                ddlOnlineMobileCode.DataBind();
            }
            if (ds.Tables[3] != null && ds.Tables[3].Rows.Count > 0)
            {
                ddlPCon.DataSource = ds.Tables[3];
                ddlPCon.DataValueField = "COUNTRYNO";
                ddlPCon.DataTextField = "COUNTRYNAME";
                ddlPCon.DataBind();
            }
            if (ds.Tables[4] != null && ds.Tables[4].Rows.Count > 0)
            {
                ddlHomeMobileCode.DataSource = ds.Tables[4];
                ddlHomeMobileCode.DataValueField = "COUNTRYNO";
                ddlHomeMobileCode.DataTextField = "COUNTRYNAME";
                ddlHomeMobileCode.DataBind();
            }
            if (ds.Tables[5] != null && ds.Tables[5].Rows.Count > 0)
            {
                ddlReceiptType.DataSource = ds.Tables[5];
                ddlReceiptType.DataValueField = "RECIEPT_CODE";
                ddlReceiptType.DataTextField = "RECIEPT_TITLE";
    ...
    protected void btnSub_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (ListViewDataItem item in lvDocument.Items)
            {
                ...
                if (lblmandatory.Text == "YES" && fuDocument.HasFile == false && docname == "")
                {
    ...
    protected void btnChallanSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int DcrTempNo = 0, checkpay=0;
            //checkpay = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TEMP", "COUNT(1)", "IDNO=" + Convert.ToInt32(Session["idno"]) + "AND (DOC_FILENAME != '') AND PAY_SERVICE_TYPE = 5 AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"]) + "and RECIEPT_CODE=" + "'" + Convert.ToString(ViewState["RECIEPT_CODE"]) + "'"));
            //DcrTempNo = Convert.ToInt32(objCommon.LookUp("ACD_DCR_TEMP", "COUNT(1)", "IDNO=" + Convert.ToInt32(Session["idno"]) + " AND PAY_SERVICE_TYPE = 5 AND SEMESTERNO=" + Convert.ToInt32(ViewState["semesterno"]) + "and RECIEPT_CODE=" + "'" + Convert.ToString(ViewState["RECIEPT_CODE"]) + "'"));
            //if (checkpay == 0)
            //if (DcrTempNo == 0)
            //{
    ...
    protected void fillDetails()
    {
        try
        {
            ...
            if (ds.Tables[9] != null && ds.Tables[9].Rows.Count > 0)
            {
                ...
                {
                    if (ds.Tables[9].Rows[i]["RECON"].ToString() == "True")
                    {
            ...
            string SP_Name2 = "PKG_USER_GET_PAYMENT_DETAILS_DOWNPAYMENT";
            string SP_Parameters2 = "@P_USERNO";
            string Call_Values2 = "" + Convert.ToInt32(Session["userno"]) + "";
            DataSet dsDeposite = objCommon.DynamicSPCall_Select(SP_Name2, SP_Parameters2, Call_Values2);
            if (dsDeposite.Tables[4].Rows.Count > 0)
            {
                lvDepositSlip.DataSource = dsDeposite.Tables[4];
    ...
    protected void lnkViewDoc_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (value == 0)
            {
                ...
                if (img != "")
                {
                    DataTable dtBlobPic = Blob_GetById(blob_ConStr, blob_ContainerNamePhoto, img);
