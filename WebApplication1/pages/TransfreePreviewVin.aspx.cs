...
public partial class ACADEMIC_TransfreePreviewVin : System.Web.UI.Page
{
    ...
    string EvalResultCount = "0"; //string global = string.Empty;
    int verified = 0, uploadedDoc = 0;
    //PDFDynamicViewer pdf = new PDFDynamicViewer();

    string blob_ConStr = System.Configuration.ConfigurationManager.AppSettings["Blob_ConnectionString"].ToString();
    string blob_ContainerName = System.Configuration.ConfigurationManager.AppSettings["Blob_ContainerNameAdd"].ToString();
    ...
    //Added By Vinay Mishra on 17/01/2024
    private bool YourConditionMethod()
    {
        // Replace this with your actual condition
        if (ViewState["EVAL_NO"].ToString() == string.Empty || ViewState["EVAL_NO"].ToString() == null || ViewState["EVAL_NO"].ToString() == "")
        {
            return true; 
        }
        else
    ...
    protected string GetClientClickCondition()
    {
        // Assuming you have some condition to check
        bool condition = YourConditionMethod();

        // Return the appropriate JavaScript function call based on the condition
        return condition ? "return AlertMsg(true);" : "return AlertMsg(false);";
    }
    //Added By Vinay Mishra on 17/01/2024 Till Here
    ...
    protected void bindOfferAcceptance()
    {
        ...
        {
            DataSet ds = null;
            ds = objdocContr.getOfferAcceptanceDetailsVin(Convert.ToInt32(Request.QueryString["userno"]), Convert.ToInt32(1), Convert.ToInt32(0));
            ...
            lblDiscipline.Text = ds.Tables[0].Rows[0]["AREA_INT_NAME"].ToString();
            lblCampus.Text = ds.Tables[0].Rows[0]["CAMPUSNAME"].ToString();
            lblFaculty.Text = ds.Tables[0].Rows[0]["COLLEGE_NAME"].ToString();
            lblAInstitute.Text = ds.Tables[0].Rows[0]["AFFILIATED_SHORTNAME"].ToString();
            lblProgram.Text = ds.Tables[0].Rows[0]["PROGRAM_NAME"].ToString();
            lblADate.Text = ds.Tables[0].Rows[0]["DEMAND_DATE"].ToString();
            lblPrevUniversity.Text = ds.Tables[0].Rows[0]["PREVIOUS_SCHOOL"].ToString();
            lblPrevProgram.Text = ds.Tables[0].Rows[0]["PREVIOUS_PROGRAM"].ToString();
        }
        catch (Exception ex)
        {
    ...
    private void ShowStudentDetails()
    {
        try
        {
            ...
            DataSet dsStudent = feeController.GetStudentInfoById_ForOnlinePayment_TransfereeVin(idno);
            if (dsStudent != null && dsStudent.Tables.Count > 0)
            {
                ...
                lblapplysemester.Text = dr["APPLYSEM"].ToString() == string.Empty ? string.Empty : dr["APPLYSEM"].ToString();
                ...
                hdnEval.Value = ViewState["EVAL_NO"].ToString();

                //if (ViewState["EVAL_NO"].ToString() == "1" && EvalResultCount == "0")
                //{
                ...
                //    divremarks.Visible = true;
                //    divEvalResult.Visible = true;
                //    EvalRemark.Enabled = false;
                //    ddlevalutionResult.Enabled = false;
                ...
                if (ViewState["EVAL_NO"].ToString() != "")
                {
                    ...
                    if (uploadedDoc == verified)
                    {
                        lblStatusREval.Visible = true;
                        evalstatus.Visible = true;
                        divremarks.Visible = true;
                        EvalRemark.Visible = true;
                        divEvalResult.Visible = true;
                        btnSubmitEval.Visible = true;
                        btnSubmitEval.Enabled = false;
                        EvalRemark.Enabled = false;
                        rdiotest.Enabled = false;
                        ddlevalutionResult.Enabled = false;
                    ...
                    else
                    {
                        ...
                        EvalRemark.Visible = false;
                        divEvalResult.Visible = false;
                        btnSubmitEval.Visible = false;
                        btnSubmitEval.Enabled = false;
                        EvalRemark.Enabled = false;
                        rdiotest.Enabled = false;
                        ddlevalutionResult.Enabled = false;
                ...
                else if (ViewState["EVAL_NO"].ToString() == "" && uploadedDoc == verified)
                {
                    lblStatusREval.Visible = false;
                    btnSubmitEval.Enabled = true;
                    EvalRemark.Enabled = true;
                    EvalRemark.Visible = true;
                    rdiotest.Enabled = true;
                    btnSubmitEval.Visible = true;
                    divEvalResult.Visible = true;
                    ddlevalutionResult.Enabled = true;
                    btnSubmit.Enabled = false;
                }
                else if (ViewState["EVAL_NO"].ToString() == "" && uploadedDoc != verified)
                {
    ...
    private void ApplicantPersonalDetails()
    {
        ...
        {
            idno = Convert.ToInt32(Request.QueryString["userno"]);
            DataSet dsStudent = objnuc.GetStudPersonalDetailsVin(Convert.ToInt32(Request.QueryString["userno"].ToString()));
            if (dsStudent != null && dsStudent.Tables.Count > 0)
            {
                DataRow dr = dsStudent.Tables[0].Rows[0];
        ...
        try
        {
            ...
            if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (ListViewDataItem row in lvDocument.Items)
                {
                    ...
                    LinkButton lnkViewDoc = row.FindControl("lnkViewDoc") as LinkButton;
                    ...
                    //Added By Vinay Mishra on 11/01/2024
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)      //Modified By Vinay Mishra on 12/01/2024
    {
        try
        {
            ...
            string docnos = "", docnames = "", status = "", remark = "", pagesno = ""; int Count = 0, reject = 0;

            DataSet dsUserContact = null;
            UserController objUC = new UserController();
            dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", Convert.ToString(ViewState["ID_NO"]), Convert.ToInt32(Request.QueryString["pageno"]));
            ...
            foreach (ListViewDataItem item in lvDocument.Items)
            {
                ...
                //if (ddlStatus.SelectedIndex == 0 && lnkViewDoc.Visible == true)
                //{
                ...
                {
                    if (lnkViewDoc.Visible == true)     //Added By Vinay Mishra on 11/01/2024
                    {
                        ...
                        if (Convert.ToString(txtRemark.Text) == string.Empty)   //Added By Vinay Mishra on 11/01/2024
                        {
                    ...
                    //else
                    //{
                    //    objCommon.DisplayMessage(this.Page, "Please Upload Document for " + DocName.Text + " !!!", this.Page);
                    //    return;
                    //}
                }
                else
                ...
                if (lnkViewDoc.Visible == true)     //Added By Vinay Mishra on 11/01/2024
                {
                    if (ddlStatus.SelectedIndex != 1 && ddlStatus.SelectedIndex == 0)
                    {
                        objCommon.DisplayMessage(this.Page, "Please Verify the Document for " + DocName.Text + " !!!", this.Page);
                        return;
                    }
                }

                if (lnkViewDoc.Visible == true && ddlStatus.SelectedIndex == 1)
                {
                    VerifiedCount += 1;
                    ddlStatus.Enabled = false;
                    txtRemark.Enabled = false;
            ...
            if (Count > 0)
            {
                ...
                if (reject == 1)
                {
                    ...
                    EmailSmsWhatssppSend(Convert.ToInt32(pagesno), lblEmailID.Text, lblStudName.Text, Convert.ToString(ViewState["ID_NO"]));        //Request.QueryString["pageno"]
                }
                CustomStatus cs = CustomStatus.Others;
                cs = (CustomStatus)objDoc.UpdateDocumentStatus_TransfereeVin(Convert.ToInt32(Request.QueryString["userno"]), docnos, docnames, status, remark, Convert.ToInt32(Session["userno"]));
                if (cs.Equals(CustomStatus.RecordSaved))
                {
                    if (UploadDocCount != VerifiedCount)
                    {
                        ShowStudentDetails();
                        evalstatus.Visible = false;
                        divremarks.Visible = false;
                        ...
                        ddlevalutionResult.Enabled = false;
                        //lblStatusREval.Visible = false;
                        objCommon.DisplayMessage(this.Page, "Document Status Saved Successfully!!! \\nBut Still Some Documents are Not Verified. Please Verify the UnVerified Documents!!!", this.Page);
                        return;
                    }
                    else
                    {
                        ...
                        ShowStudentDetails();
                        btnSubmit.Enabled = false;
                        objCommon.DisplayMessage(this.Page, "Document Status Saved Successfully !!!", this.Page);
                        return;
                    }
                }
    ...
    protected void EmailSmsWhatssppSend(int Page_No, string toSendAddre, string Name, string IDNO)
    {
        ...
        {
            UserController objUC = new UserController();
            DataSet dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", IDNO, Page_No);
    ...
    protected void lnkSendEmail_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Pay > 0)
            {
                ...
                DataSet dsUserContact = null;
                UserController objUC = new UserController();
                dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", Convert.ToString(Request.QueryString["userno"]), Convert.ToInt32(Request.QueryString["pageno"]));
    ...
    protected void EmailSmsWhatssppSend(int Page_No, string userno, string Message, string toEmailId, string sub, string name, string degreename, string campusname, string weekdayname, string date, int idno, string College_Code)
    {
        ...
        if (Convert.ToInt32(ds.Tables[0].Rows[0]["EMAIL"].ToString()) == 1)//For Email Send 
        {
            ...
            //int PageNo = 33;
            UserController objUC = new UserController();
            DataSet dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", userno.ToString(), PageNo);
            if (dsUserContact.Tables[1] != null && dsUserContact.Tables[1].Rows.Count > 0)
            {
                string Subject = dsUserContact.Tables[1].Rows[0]["SUBJECT"].ToString();
                Message = dsUserContact.Tables[1].Rows[0]["TEMPLATE"].ToString();
    ...
    protected void btnRefundInitiated_Click(object sender, EventArgs e)
    {
        ...
        string subject = "";
        string filename = "";
        dsUserContact = user.GetEmailTamplateandUserDetailsVin("", "", "ERP", Convert.ToInt32(2995));
        if (cs.Equals(CustomStatus.RecordSaved))
        {
    ...
    protected void ddlevalutionResult_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            rdiotest.SelectedValue = "0";
            EvalRemark.Text = string.Empty;
            evalstatus.Visible = false;
            divremarks.Visible = false;
            btnSubmitEval.Visible = true;
        }
    ...
    protected void btnSubmitEval_Click(object sender, EventArgs e)
    {
        ...
        string SP_Name1 = "PKG_ACD_INSERT_EVALUTION_RESULT_VIN";
        ...
        if (que_out1 == "1")
        {
            ...
            {
                PageNo = 287990;
                //dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", Convert.ToString(ViewState["ID_NO"]), Convert.ToInt32(287990));
                //Subject = dsUserContact.Tables[1].Rows[0]["SUBJECT"].ToString();
                //message = dsUserContact.Tables[1].Rows[0]["TEMPLATE"].ToString();
            }
            else
            {
                PageNo = 287991;
                //dsUserContact = objUC.GetEmailTamplateandUserDetailsVin("", "", Convert.ToString(ViewState["ID_NO"]), Convert.ToInt32(287991));
                //Subject = dsUserContact.Tables[1].Rows[0]["SUBJECT"].ToString();
                //message = dsUserContact.Tables[1].Rows[0]["TEMPLATE"].ToString();
            }
